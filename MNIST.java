...
public class MNIST {
    ...
    public static void main(String[] args) {
        try {
            String path = "C://AEGIS//roadmap//Deep_Learning//mnist//data//MNIST//raw";
            ...
            NeuralNetwork model = new NeuralNetwork(new Layer[]{l1,l2}, new CrossEntropy(),new SGD(0.01));
            int epochs = 10;
            int batchSize = 64;

            for(int i = 0; i < epochs; i++) {
                double loss = model.train(XTrain,YTrain,batchSize);
                double trainAccuracy = accuracy(model.forward(XTrain),YTrain);
                double testAccuracy = accuracy(model.forward(XTest),YTest);
                System.out.println("Epoch: " + (i + 1) + " Loss: " + loss + " Train Accuracy: " + trainAccuracy + " Test Accuracy: " + testAccuracy);
            }
    ...
    private static double accuracy(Matrix prediction,Matrix y) throws InvalidValue {
        ...
        for(int i = 0; i < N; i++) {
            ...
            int actual = 0;
            for(int j = 1; j < 10; j++) {
                if(prediction.get(i,j) > prediction.get(i,predicted))
                    predicted = j;
                if(y.get(i,j) > y.get(i,actual))
                    actual = j;
            }
            if(predicted == actual)
                correct++;
